...
if (isset($_POST["action"])) {
    ...
    if(isset($_POST['android_id_prospecto'])){
        $_SESSION['alumno']['id_prospecto'] = $_POST['android_id_prospecto'];
    }
    ...
    switch ($accion) {
        ...
        case 'list_plans':
            ...
            require_once(__DIR__."/../../functions/conekta-php/lib/Conekta.php");
            require_once("../../functions/api_key_conekta.php");
            $lista = \Conekta\Plan::find('');
            // var_dump($lista);
            //foreach ($lista as $key => $plan) {
                // $MPlan = \Conekta\Plan::find($plan->id);
                // var_dump($MPlan->delete());
                // echo "<br>";
                // echo "<br>";
            //}
            break;
	    ...
	    require_once '../../functions/correos_prospectos.php';
            ...
            if ($info_concepto['categoria'] == 'Mensualidad' && $_POST['domiciliar_tarjeta']==1 && $info_pago['monto_por_pagar'] == $info_pago['monto_total_concepto']) { //si se esta pagando por concepto de mensualidad y se selecciono opcion de domiciliar tarjeta suscribimos al cliente al plan de pagos conekta con el id que tiene el concepto al crear el plan de pagos
                /* crear plan de pago en conekta para concepto de mensualidad */
                // revisar que no haya subscripcion activa
                require_once '../../Model/planpagos/domiciliacionModel.php';
                $domM = new Domiciliar();
                $subscripcion = $domM->info_plan($id_prospecto, $info_concepto['id_generacion'])['data'];
                if($subscripcion['plan_domiciliacion'] !== null){
                    echo json_encode('Este concepto ya se encuentra domiciliado');
                    die();
                }
                $id_plan_conekta = false;
                try{
                    $plan = \Conekta\Plan::create(array(
                        'name' => $info_concepto['concepto'],
                        'amount' => $info_pago['monto_por_pagar']*100,
                        'currency' => strtoupper($currency),
                        'interval' => 'month',
                        'frequency' => 1,
                        'expiry_count' => $info_concepto['numero_pagos'] - $info_pago['numero_de_pago'] 
                    ));
                ...
                try{
                    $cliente = (object) ['subscription'=>['status'=>false]];
                    if($monto !== false){
                        $cliente = \Conekta\Customer::create(
                            array(
                        ...
                        // guardar en base de datos el id del plan al que se suscribe
                        $domM->actualizar_plan($id_prospecto, $info_concepto['id_generacion'], $id_plan_conekta, $cliente->id);
                    }
                }catch (\Conekta\ProccessingError $error){
                    echo "ProccessingError" . $error->getMessage();
                } catch (\Conekta\ParameterValidationError $error){
                    echo "ParameterValidationError" . $error->getMessage();
                } catch (\Conekta\Handler $error){
                ...
                if ($cliente->subscription->status=="active") {
                    $plan_pago_exitoso=true;
                    $ord_id = $cliente->subscription->last_billing_cycle_order_id;
                    $ult_ord = \Conekta\Order::find($ord_id);
                    $method_pay = $ult_ord->charges[0]->payment_method->type;
                    $auth_cod = $ult_ord->charges[0]->payment_method->auth_code;
                }
                ...
                if($order->payment_status=="paid") {
                    $orden_pago_exitoso=true;
                    $ord_id=$order->id;
                    $auth_cod = $order->charges[0]->payment_method->auth_code;
                    $method_pay = $order->charges[0]->payment_method->type;
                }
            }
            if ($orden_pago_exitoso || $plan_pago_exitoso) {
                echo "1";
                ...
                if (!$tiene_pagos_verificados && $info_concepto['id_generacion'] > 0) {
                    ...
                    if($info_concepto['id_generacion'] > 0){
                ...
                if(!$plan_pago['data']){
                    ...
                    //if(isset($_SESSION['alumno']['id_prospecto']) ){
                        foreach ($resp['data']['pagos_aplicar'] as $key => $value) {
                            ...
                            if($value['idExamen'] != null && $value['idExamen'] > 0){
                                if(!isset($_POST['from_funct'])){
                                require_once '../../Model/controlescolar/examenModel.php';
                                require_once '../../Model/controlescolar/controlEscolarModel.php';
                                }
                                $exmM = new Examen();
                                $controlE = new ControlEscolar();
                                $info_e = $exmM->cargar_examen_id($value['idExamen']);
                                if($info_e['data']){
                                    $lista_aplicar = $controlE->alumnos_examen($value['id_generacion'], ['cursoExamen'=>$info_e['data']['idCurso']])['data'];
                                    $alumno_aplica = array_search($alumno_id, array_column($lista_aplicar, 'idAsistente'));
                                    if($alumno_aplica === false){
                                        unset($resp['data']['pagos_aplicar'][$key]);
                                    }
                                }
                            }
        ...
        case 'pago_prospecto':
            unset($_POST['action']);
            $currency='mxn';
            if(isset($_POST['tipo_pago']) && $_POST['tipo_pago'] > 0 && isset($_POST['person_pago']) && isset($_POST['inp_monto_pago']) && isset($_POST['inp_fecha_pago'])){
        ...
        case 'consultar_ofertas':
            ...
            break;
        case 'generarSPEI': // Solo para inscripciones
            $info_pago = $pagosM->obtener_pago_aplicar($_POST['persona'], $_POST['concepto'], date('Y-m-d'));
            /** */
            $_POST['id_concepto']   = $_POST['concepto'];
            require_once(__DIR__."/../../functions/conekta-php/lib/Conekta.php");
            require_once("../../functions/api_key_conekta.php");
            $datos_concepto=$pagosM->obtener_concepto($_POST['concepto']);
            if($datos_concepto['data']['categoria'] != 'Inscripción'){
                echo json_encode(['estatus'=>'error', 'info' => 'Este servicio solo está disponible para inscripciones']);
                die();
            }
            
            $id_concepto_pago   = $_POST['concepto'];
            $id_promocion       = $info_pago['id_promocion'];
            $nombre_concepto    = $datos_concepto['data']['concepto'];
            $monto_pago         = $info_pago['monto_por_pagar'];

            $monto_recargo_pagado = @$_POST['monto_recargo_pagado'];

            $currency='mxn';
            $id_prospecto = $_POST['persona'];
            $info_alumn = $prospectoM->consultar_info_prospecto_a($_POST['persona'])['data'];
            $datosprospecto=$pagosM->obtener_datos_prospecto($id_prospecto);
            ...
            echo json_encode(['estatus'=>'error', 'info'=>$error->getMessage()]);
            die();
            } catch (\Conekta\Handler $error){
            echo json_encode(['estatus'=>'error', 'info'=>$error->getMessage()]);
            die();
            ...
            break;
        case 'consultar_pagos_anteriores':
            echo json_encode($pagosM->listar_pagos_anteriores($_POST['concepto'], $_POST['prospecto']));
            break;
        default:
            # code...
            break;
