...
class documentos{
	...
	function consultarDocumentosListaCompleta(){
		...
		if($con['info'] == 'ok'){
			...
			$sql = "SELECT * FROM listado_documentos";
	...
	public function consultarInfoDocumento($nombreDocumento){
		...
		if($con['info'] == 'ok'){
			...
			$sql = "SELECT * FROM listado_documentos WHERE nomenclatura_documento = :nombreDocumento";
	...
	public function consultarDocumentosVerificacion($idAfiliado){
		...
		if($con['info'] == 'ok'){
			...
			$sql = "SELECT ld.letrero, ld.id_documento AS idVerificar, ld.nombre_documento, ld.nomenclatura_documento,ld.nacionalidad ,doc.*  FROM listado_documentos ld
					LEFT JOIN documentos as doc ON doc.id_prospectos = :idAfiliado AND ld.id_documento = doc.id_documento
					WHERE ld.verificacion = 1 GROUP BY ld.id_documento;";
