			...
			try{
				...
				if(cursos_disp.estatus == 'ok'){
					...
					for (i = 0; i < cursos_disp.data.length; i++) {
						...
						if(cursos_disp.data[i].idInstitucion == 20){
...
$("#select-clase").on('change', function(){
	...
	if(clase){
		...
		$("#material_apoyo").html((lis!='')?lis:'<li>No hay material de apoyo disponible</li>')

		if(clase.video != ''){
			$("#viedo_link").attr("href","reproductor2.php?url="+clase.video)
	...
	for (i = 0; i < tarea.entregas.length; i++) {
		...
		html_t += `<div class="row py-3 border border-top-0 border-right-0 border-left-0 border-secondary">
                    ...
                    <div class="col-sm-6 ml-auto pt-1">
                        <span>Calificación: ${(t.calificacion === null)? 'pendiente' : (t.calificacion)}</span>
                      <div class="progress">
		...
		success: function(data){
			try{
				...
				for (i = 0; i < examenes_r.length; i++) {
					console.log(examenes_r[i].idExamen+" "+Usuario+" "+"Verificando");
					if(examenes_r[i].idExamen != 0 && Usuario != 7504){
						buton_presentar = '';
						if (examenes_r[i].presentaciones.length > 0) {
							if(!HIDE_OPTIONS){
								...
								buton_presentar = `<span  class="bg-primary text-light p-1 px-2">Ya has presentado este examen</span>`
							}
							...
							if(aplicar_nuevamente){
								...
								if(examenes_r[i].ontime){
									...
									buton_presentar += `<button class="btn btn-sm btn-secondary">Examen vencido No UNICO</button>`
								}
							}else{
						...
						if (examenes_r[i].presentaciones.length > 0) {
							...
							if(examenes_r[i].ontime){
								...
								if(examenes_r[i].before){
									...
									buton_presentar = `<button class="btn btn-sm btn-secondary">Examen vencido y unico</button>`
								}
							}
						}
					}
...
$.ajax({
	...
	success: function(data){
		...
		html_c=`<div class="col-sm-6 col-md-6 col-xl-4">
			...
			switch(data.trim()){
				case 'no inscripcion':
					mensaje = `<small>${nombreCarrera}</small><br>Su acceso se ha bloqueado por no contar con registro de pago de inscripción`;
				break;
				case 'no mensualidad':
					mensaje = `<small>${nombreCarrera}</small><br>Su acceso se ha bloqueado por no contar con registro de pago de mensualidad`;
				break;
				case 'no reinscripcion':
					mensaje = `<small>${nombreCarrera}</small><br>Su acceso se ha bloqueado por no contar con registro de pago de reinscripción`;

				break;
				case 'no documentos':
					mensaje = `<small>${nombreCarrera}</small><br>Su acceso se ha bloqueado falta de entrega de documentos digitales`;
				break;
				case 'no documentos fisicos':
					mensaje = `<small>${nombreCarrera}</small><br>Su acceso se ha bloqueado falta de entrega de documentos fisicos`;
				break;
				case 'Alumno_con_baja':
					mensaje = `<small>${nombreCarrera}</small><br>Su acceso se ha bloqueado porque se encuentra dado de baja para este curso, contacte a control escolar`;
				break;
			...
			if(mensaje != ''){
				$("#cursos-container").append(`<div class="alert alert-warning mx-2" style="max-height: 100px;" role="alert">
		...
		success: function(data){
			try{
				...
				$.each(data, function(i, item) {
					var semestre = i +1;
					$('#mostrar-periodos').append("<table class='table table-colored table-dark' > <thead><th colspan='2' class='text-center'>"+periodo+": "+ semestre +"</th></thead> <tr><th  class='wd-10p text-center' id='materias"+i+"'>Materias</th><th class='wd-10p text-center' id='calificacion"+i+"'>Calificaciones</th></tr> <tbody></tbody></table>")
						...
						$('#materias'+i).append("<tr style='white-space: break-spaces;'  class='wd-10p'><th>"+data[i].materias_calificadas[j].nombre+"</th> </tr>");
						$('#calificacion'+i).append("<tr style='white-space: break-spaces;'  class='wd-10p'><td style='white-space: break-spaces;'  class='wd-10p text-center'>&nbsp;&nbsp;&nbsp;&nbsp;"+data[i].materias_calificadas[j].calificacion+"</td></tr>")
						}); 
