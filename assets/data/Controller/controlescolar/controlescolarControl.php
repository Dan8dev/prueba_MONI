...
if (isset($_POST["action"])) {
    ...
    switch($_POST['action']){
        case 'ActualizarGrupo':
            ...
            foreach(array_keys($_POST['idAlums']) as $key){
                $_POST['idAlum'] = $_POST['idAlums'][$key];
                $actualGrupo = $ce->ActualizarGrupo($_POST);
            }
            echo json_encode($actualGrupo);
            break;
        ...
        case 'consultarAsistenciaEventos':
            ...
            while($dato=$loadAlumnos->fetchObject()){
                ...
                0=> $dato->titulo,
                1=> $dato->fechaE,
                2=> "<a href='lista.php?t=ev&id_evento={$dato->id_evento}' target='_blank' class='btn btn-primary'>Ver Lista</a> <button class='btn btn-primary' onClick = 'verAsistencia({$dato->id_evento})'>Ver Asistentes</button>"
                );
            }
            $result = array(
                'sEcho'=>1,
        ...
        case 'consultarMaestros':
            ...
            $loadAlumnos = $ce->consultarMaestros($id);
            $data = Array();
            while($dato=$loadAlumnos->fetchObject()){
        ...
        case "editarMaestro":
            unset($_POST['action']);
            // OBTENER_INFO MAESTRO
            $maestro = $ce->buscarMaestro($_POST['idMaestro'])['data'][0];
            // var_dump($maestro);
            // var_dump($_POST);
            if($maestro['email'] != $_POST['email_em']){
                // validar disponibilidad de correo nuevo
                $verify_mail = $accM->get_info_maestro_correo($_POST['email_em']);
        ...
        case 'listarCarrerasE':
            unset($_POST['action']);
            $id = $_SESSION['usuario']['estatus_acceso'];
            $carreras= $ce->buscarCarrerasE($_POST['idBuscar'],$id);
            echo json_encode($carreras);
            break;
        break;
        ...
        case 'listarCarrerasAsignacion':
            unset($_POST['action']);
            $id = $_SESSION['usuario']['estatus_acceso'];
            $carreras= $ce->buscarCarrerasAsignacion($_POST['idBuscar'],$id);
            echo json_encode($carreras);
            break;
        break;
        ...
        case 'selectSessions':
            ...
            foreach($datas as $data){
                ...
                $date = $data['fecha_hora_clase'];
                $title = $data['titulo'];
                $teachers = $data['teachers'] != NULL && trim($data['teachers']) != "" ? "<b> {$data['teachers']} </b>" : 'Sin docente asignado'; 
                $correo  = $data['correo'] != NULL && trim($data['correo']) != "" ?  "{$data['correo']}" : "Sin información de correo";
                ...
                $resp[] = array(
                    ...
                    4 => $correo,
                    5 => $data['nCar'],
                    6 => 'Generación'.$data['sGen'],
                    7 => '<button class="btn btn-primary" data-toggle="modal" data-target="#modalEditSession" onclick="editSessions(`'.$title.'`,'.$data['idClase'].',`'.$date.'`);">Editar</button>', 
                );
        ...
        case 'buscarCarreras':
            unset($_POST['action']);
            $id = $_SESSION['usuario']['estatus_acceso'];
            $loadCarreras = $ce->buscarCarreras($id);
            $data = Array();
        ...
        case "agregarMaestro":
            unset($_POST['action']);
            // email_am
            $verify_mail = $accM->get_info_maestro_correo($_POST['email_am']);
            if($verify_mail !== false){
                echo json_encode(['estatus'=>'error', 'info'=>'Este correo ya está siendo utilizado por otro docente']);
                die();
            }
            $agregar = $ce->agregarMaestro( $_POST );
            if($agregar['estatus'] == 'ok' && $agregar['data'] > 0){
                enviar_accesos($_POST['aPaterno_am'].' '.$_POST['nombres_am'], $_POST['email_am']);
            }
        ...
        case 'consultarAlumnos':
            ...
            while($dato=$loadAlumnos->fetchObject()){
                ...
                3=> $dato->telefono,
                4=> $dato->docs,
                5=> "<button class='btn btn-primary' onclick='registrarDocumentacion({$dato->id_afiliado},{$dato->idGeneracion},{$dato->pais})'>Documentación Fisica</button> ".
                    "<button class='btn btn-success' onclick='asignarProrrogaAlumno({$dato->id_afiliado},{$dato->idGeneracion})'>{$texto}</button> ".
                    "<button class='btn btn-secondary' onclick='validarVistaDocumentos({$dato->id_afiliado},{$dato->idGeneracion},{$dato->pais})'>Subir Documentos</button> ".
                ...
                6=> $Carrera == 13 ? ($dato->Diplomado == 2 ? "<button  class='btn btn-success'>Diplomado</button>": "<button  class='btn btn-primary'>Alumno TSU</button> <button  class='btn btn-success' onClick = 'CambiarTipo({$dato->idAlumnoGeneracion})'>Cambiar a Diplomado</button>") : '<button class="btn btn-success">Alumno</button>',
        ...
        case 'consultarExpediente':
            ...
            while( $dato = $loadAlumnos->fetchObject()){
                ...
                //Verificiacion de la existencia del documento en otro dominio
                if(!file_exists($retroceso_controller.$link_documento)){
                    $link_documento = 'https://conacon.org/siscon/app/lista_documentos/'.$dato->id_prospectos.'/'.$dato->nombre_archivo;
                }
                ...
                $data[]=array(
                0=> '<input type="hidden" id="iddocumento'.$dato->id_documento.'" name="iddocumento'.$dato->id_documento.'" value="'.$dato->id.'"></input><b>'.$dato->nombreDoc.'</b><br>'.$comentario,
                1=> $val,
        ...
        case 'consultarClasesMaestros':
            unset($_POST['action']);
            $id = $_SESSION['usuario']['estatus_acceso'];
            $loadClases = $ce->consultarClasesMaestros( $_POST['idBuscar'],$id );
            $data = Array();
            ...
            case 'consultarExamenes':
                ...
                if(isset($_POST['vista']) && $_POST['vista']!=2){
                    $controlButton = true;
                    $url = '';
                }else{
                    $url = '../controlescolar/';
                ...
                while($dato=$loadAlumnos->fetchObject()){
                    $Button = " ";
                    if($controlButton){
                        $Button = '<button type="button" class="btn btn-secondary" onclick="editarExamen('.$dato->idExamen.')">Editar</button>';
                    }
                    
                    $fecha_f = date($dato->fechaInicio);
                    $fh = strtotime($fecha_hoy);
                    $ff = strtotime($fecha_f);
                    
                    if($fh >= $ff){
                        $link = '<a href="'.$url.'resultadoExamen.php?idExamen='.$dato->idExamen.'" target="_blank" class="btn btn-primary">Ver resultados</a> ';
                        $disabled = '';
                    }else{
                        $link = '<button target="_blank" class="btn btn-primary" disabled>Ver resultados</button>';
                        $disabled = 'disabled';
                    }
                    ...
                    4=> $dato->fechaFin,
                    5=> $link.$Button.
                    ' <button class="btn btn-primary" onclick="revisar_entregas('.$dato->idExamen.')" '.$disabled.'>
        ...
        case 'listarCarrerasExpedientes':
            ...
            $idAcc = $_SESSION['usuario']['estatus_acceso'];
            $carreras= $ce->listarCarrerasExpedientes($idAcc);
            echo json_encode($carreras);
            break;
        break;
        ...
        case 'buscarClasesCarrera':
            ...
            if(!isset($_POST['vista'])){
                $_POST['vista'] = $_SESSION['usuario']['estatus_acceso'];
            }

            $obClases = $ce->buscarClasesCarrera($_POST)['data'];
...
//mike
                ...
                if(!empty($_POST['gradoEstudio']) && $_POST['gradoEstudio'] != 'undefined'){
                    if($_POST['documento']==4){
                        ...
                        if($pdfs == 0){
                            ...
                            if(!file_exists('../../../../udc/app/lista_documentos/'.$id)){
                                mkdir('../../../../udc/app/lista_documentos/'.$id, 0707);
                            }
        
                            $tmp_name = $_FILES['file']['tmp_name'];
                            $uploads_dir = "../../../../udc/app/lista_documentos/".$id;
        ...
        case 'crearExamen':
            ...
            unset($_POST['datatable-tablaPreguntas2_length']);
             ...
             //Aplicar una bandera cuando un examen es extaroridnario o no para que solo traiga los alumnos que reprobaron
            ...
            //Verifica el envio de un id_correcto
                ...
                if(isset($_POST['aplicar_extraordinario']) && $_POST['aplicar_extraordinario'] == 2){
                    $_POST['aplicar_extraordinario'] = ['cursoExamen'=>$_POST['cursoExamen']];
                }else{
                    $_POST['aplicar_extraordinario'] = 1;
                }

                $alumnos_examen = $ce->alumnos_examen($_POST['examenGeneracion'],$_POST['aplicar_extraordinario'])['data'];
                
                                    require_once '../../functions/correos_prospectos.php';
                
                if(sizeof($alumnos_examen)>0){
        ...
        case 'editarExamen':
            ...
            case 'CrearExamenBanco':
    		 ...
    		 unset($_POST['aplicar_ordinarioBanco']);
                ...
                if(isset($_POST['aplicar_extraordinarioBanco']) && $_POST['aplicar_extraordinarioBanco'] == 2){
                    $_POST['aplicar_extraordinarioBanco'] = ['cursoExamen'=>$_POST['cursoExamenBanco']];
                }else{
                    $_POST['aplicar_extraordinarioBanco'] = 1;
                }

                $alumnos_examen = $ce->alumnos_examen($_POST['examenGeneracionBanco'],$_POST['aplicar_extraordinarioBanco'])['data'];
                ...
                if(sizeof($alumnos_examen)>0){
                    ...
                    $liga_institucion = ($udc_o_conacon==20)?'https://moni.com.mx/udc/':'https://conacon.org/moni/siscon/';
                    foreach ($alumnos_examen as $key => $value) {
                            $asunto = "RECORDATORIO ".$_POST['nombreExamenBanco']." - ".$institucion;
                            $destinatarios = [[$value['email'], $value['nombre_completo']]];
                            $plantilla_c = 'plantilla_recordatorio_examen.html';
                            $claves = ['%%prospecto','%%instituto','%%nombreexamen','%%horainicioexamen','%%horafinexamen','%%nombre_carrera','%%logo_institucion','%%liga_institucion_examen'];
                            $valores = [$value['nombre_completo'],$institucion,$_POST['nombreExamenBanco'],$_POST['fechaInicioExamenBanco'],$_POST['fechaFinExamenBanco'],$value['nombre'],$logoinstitucion,$liga_institucion];
                            $enviar = enviar_correo_registro($asunto, $destinatarios, $plantilla_c, $claves, $valores, "none");
                    }

                    $obtener_maestro = $ce->obtener_maestro($_POST['selectMaestrosBanco'])['data'];
                    $nombre_carrera = $ce->obtener_nombre_carrera($_POST['CarreraBanco'])['data'];
                    $asunto = "RECORDATORIO ".$_POST['nombreExamen']." - ".$institucion;
                            $destinatarios = [[$obtener_maestro['email'], $obtener_maestro['nombre_completo']]];
                            $plantilla_c = 'plantilla_recordatorio_examen_maestro.html';
                            $claves = ['%%prospecto','%%instituto','%%nombreexamen','%%horainicioexamen','%%horafinexamen','%%nombre_carrera','%%logo_institucion'];
                            $valores = [$obtener_maestro['nombre_completo'],$institucion,$_POST['nombreExamenBanco'],$_POST['fechaInicioExamenBanco'],$_POST['fechaFinExamenBanco'],$nombre_carrera['nombre'],$logoinstitucion];
                            $enviar = enviar_correo_registro($asunto, $destinatarios, $plantilla_c, $claves, $valores, "none");
                }
                break;
...
// chuy
        case 'VerificarDocumentosAlumno':
            unset($_POST['action']);
            $cDatos = $ce->consultarDocumentosList($_POST['idAlum']);
        ...
        case 'RegistroDocumentosExtranjeros':
            ...
            //                 "Comprobante_Domicilio"=>["comprobante_domicilio",11],
            //                 "CertEstMedicina1"=>["comprobante_estudios1",4],
            ...
            $indexNombres = [];
            $indexNombresConsulta = $ce->consultarDocumentosListaCompleta();
            ...
            foreach($_FILES as $index => $Archivos){
                if($Archivos['tmp_name'] != null && $Archivos['tmp_name'] != ""){
                    if($Archivos['size'] > $maxSize){
                        ...
                        $Direccion = "../../../../udc/app/lista_documentos/$id/";
        ...
        case 'consultarDocumentosListaCompleta':
            unset($_POST['action']);
            $infoDocumentos = $ce->consultarDocumentosListaCompleta();
        ...
        case 'actualiza_clase':
            ...
            if(isset($_POST['oldFilesMat']) && $_POST['oldFilesMat'] != ''){

                $oldfiles = json_decode($_POST['oldFilesMat']);
            
                foreach($oldfiles as $key){
                    array_push($materiales,$key);
                }
            }
            if(isset($_POST['oldFilesRec']) && $_POST['oldFilesRec'] != ''){

                $oldfiles = json_decode($_POST['oldFilesRec']);
                foreach($oldfiles as $key){
                    array_push($recursos,$key);
                }
            }
            ...
            unset($_POST['oldFilesMat']);
            unset($_POST['oldFilesRec']);
        ...
        case 'registrarDocumentosFisicos':
            ...
            $fRegistro = $_POST['fechaAct'];
            unset($_POST['fechaAct']);

            $HourRegistro = date('H:i:s');
             
            $fRegistro = "{$fRegistro} {$HourRegistro}";
            ...
            if(count($_POST) == 2){
                ...
                $Borrar = array_diff($ids, $listChecks);
                $Nuevos = array_diff($listChecks, $ids);
                
                //array para borarr ldocuemntos que se hayan quitado
                foreach($Nuevos as $Regist){
                    $ce->registrarDocumentosFisicos($Regist,$_POST['idAlumnoDocumentacionFisica'],$fRegistro, $_POST['idUsuarioRegistro']);
                    }

                //Array para quitar registros nuevos
                foreach($Borrar as $Quite){
                    $ce->reseteoDocumentosEspecificosFisicos($_POST['idAlumnoDocumentacionFisica'],$Quite);
                }
            
                $response = ['estatus'=>'ok', 'data'=>'633111'];
                echo json_encode($response);
                //Función amntigua de reseteo y registro
        ...
        case 'ObtenerControlEscolar':
            unset($_POST['action']);
            $SelectControlEscolar = $ce->ObtenerControlEscolar();
            echo json_encode($SelectControlEscolar);
            break;
        //fin_mike
        ...
        case 'consultarAsistenciaMaterias':
                ...
                case 'consultarAlumnosGen':
                            ...
                            $data = Array();
                                while($dato=$Genciclo->fetchObject()){
                                    ...
                                    $PromedioAlum = $ce->CalculoDePromedioGeneraL(["idGen"=>$Generacion,"idAlum"=>$dato->idalumno]);
                                    //var_dump($PromedioAlum);
                                    $data[]=array(
                                        0=> $dato->nombre,
                                        1=> $dato->Matricula,
                                        2=> count($NumeroCiclos['data']) > 0 ? $PromedioAlum["data"] ? $PromedioAlum["data"]["promedio"] == 10 || $PromedioAlum["data"]["promedio"] == 0 ? number_format($PromedioAlum["data"]["promedio"],0) : number_format($PromedioAlum["data"]["promedio"],2) : "<b>SIN PROMEDIO GENERAL</b>" : "<b>SIN PROMEDIO GENERAL</b>",
                                        3=> count($NumeroCiclos['data']) > 0 ? "<a type='button' target='_blank' href= 'GenerarKardex.php?Generacion=$Generacion&idCarrera=$Carrera&idAlumno=$dato->idalumno&nombre=$dato->nombre'>Generar Kardex</a>" : "<h5>SIN CALIFICACIONES</h5>"
                                        );
                                }
                                //<button type="button" class="btn btn-primary" onclick="editarExamen('.$dato->idExamen.')">Editar</button> 
                                $result = array(
            ...
            case 'ActualizarAsignadosArticulo':
                ...
                //Variable para verificar la activacion de Servicio social en lapagina correspondiente
                $idinstitucion = $ce->BuscarIdInstitucion(['idCarr'=>$_POST['idCarr']])['data'];
                unset($_POST['idCarr']);
                $_POST['institucion'] = $idinstitucion['idInstitucion'];
            ...
            case 'EliminarProcesoRec':
                ...
                $EliminarFormatosProc = $ce->EliminarFormatoRec($_POST);
            ...
            case 'DocumentosAlumnosServicioCorr':
                ...
                while($dato=$documentosCorregidos->fetchObject()){
                    ...
                    $data[]=array(
                        ...
                        2=> '<button class ="btn btn-primary form-group" onClick ="agregarObservaciones('.$dato->iddocumento.','.$dato->idAfiliado.',\''.$dato->nombre.'\')">Ver Documentos</button>',
                        );
                }
                //<button type="button" class="btn btn-primary" onclick="editarExamen('.$dato->idExamen.')">Editar</button> 
                $result = array(
            ...
            case 'DocumentosAlumnosServicioSolic':
                ...
                $data = Array();
                while($dato=$documentosCorregidos->fetchObject()){
                    $ButtonDoc = '<button class ="btn btn-primary" onClick ="agregarObservaciones('.$dato->iddocumento.','.$dato->idAfiliado.',\''.$dato->nombre.'\')">Ver Documentos</button>';
                    switch($dato->estatusServ){
                        case '3':
                             $ButtonOrig = '<button class ="btn btn-success" >Originales Solicitados</button>';
                            ...
                            break;
                        case '4':
                            $ButtonConcluir = '<button class ="btn btn-success"><b>Originales Aprobados</b></button>';
                            $ButtonOrig ="";  
                            break;
                        default:
                        ...
                        while($dato=$Model2->fetchObject()){
                            ...
                            for($i=0;$i<$dato->vecesenvio;$i++){
                                ...
                                if($Consulta['data'] != []){
                                    ...
                                    switch($Consulta['data'][0]['estatus']){
                                        case '1':
                                            ...
                                            break;
                                        case '2':
                                            $Mensaje = "<b class = 'text-primary'>REVISIÓN</b>";
                                            break;
                                        case '3':
                                            $Mensaje = "<b class = 'text-success'>APROBADO</b>";
                                            break;
                                    }
                                    $Buttons = '<div class = "row  m-0 justify-content-center"><button class ="btn btn-primary form-group" onClick ="formatoListo('.$idDocumento.')">Aprobar Documento</button> <button class ="btn btn-primary form-group" onClick ="MostrarDocumentosAlumno('.$idDocumento.',\''.$idUsuario.'\')">Hacer Observaciones</button></div>';
                                ...
                                $data[]=array(
                                    ...
                                    4=> $ControlDoc == '' ? '<div class = "row text-center form-group"><div class = "col-md-12">Inactivo</div></div>' : $ControlDoc
            ...
            case 'consultarDocumentosAlumnosServicio':
                ...
                while($dato=$documentosAlServicio->fetchObject()){
                    ...
                    $data[]=array(
                        0=> '<strong>Alumno: </strong>'.$dato->nombre.'<br><strong>Carrera: </strong>'.$dato->NombreCarr.'<br><strong>Generacion: </strong>'.$dato->nombreGen,
                        1=> '<strong>Proceso: </strong>'.$dato->nombreproc.'<br><strong>Formato: </strong> '.$dato->nombredocproc.'<br><strong>Número de formato enviado: </strong>'.$dato->intento.'<br><strong>Archivo Enviado: </strong>'.$dato->nombredocAlum,
                        2=> '<div class = "row  m-0 justify-content-center"><button class ="btn btn-primary form-group" onClick ="agregarObservaciones('.$dato->iddocumento.','.$dato->idAfiliado.',\''.$dato->nombre.'\')">Ver documentos</button></div>',
                        );
                }
                //<button type="button" class="btn btn-primary" onclick="editarExamen('.$dato->idExamen.')">Editar</button> 
                $result = array(
                ...
                $data = Array();
                    ...
                    while($dato=$SolicitudesCred->fetchObject()){
                        ...
                        if($dato->foto != null || $dato->foto != ""){
                            ...
                            //var_dump(file_exists($direccion),$direccion);
                            //var_dump(dirname(__FILE__));
                            if(file_exists("$direccion")){
                                $fotografia = '<div class = "row  m-0 justify-content-center"><img class="img-responsive" width="150" height="180"  src="../'.$carpeta.'/app/lista_documentos/'.$dato->idalumno.'/'.$dato->foto.'"></div>';
                            }else{
                                $fotografia = '<div class = "row  m-0 justify-content-center"><img class="img-responsive" width="200" height="150"  src="../assets/images/default-1.png"></div><div class = "row  m-0 justify-content-center"><b>FOTOGRAFIA NO ENCONTRADA</b></div>';
                            }
                            
                            if($dato->validacion == '2'){
                                $fotografia = '<div class = "row  m-0 justify-content-center"><b>FOTOGRAFIA NO VALIDADA</b></div>';
                            }else{
                                if($dato->validacion == '0'){
                                    $fotografia = '<div class = "row  m-0 justify-content-center"><b>FOTOGRAFIA EN ESPERA DE REVISIÓN</b></div>';
                                }   
                            }
                        }
                        ...
                        $Sol2=4;
                        $data[]=array(
                            0=> $fotografia !='' ? $fotografia : '<div class = "row  m-0 justify-content-center"><b>FOTOGRAFIA PENDIENTE</b></div>',
                            ...
                            3=> '<div class = "row  m-0 justify-content-center">'.$Button.'</div>'
                            );
            ...
            case 'consultarAlumnosGenTitulados':
                ...
                $data = Array();
                    while($dato=$Genciclo->fetchObject()){
                        ...
                        switch ($dato->estatus) {
                            case 2:
                                $Mensaje = "ALUMNO CON BAJA";
                                break;
                            case 3:
                                $Mensaje = "ALUMNO EGRESADO";
                                break;
                            case 4:
                                $Mensaje = "ALUMNO TITULADO";
                                break;
                            case 5:
                                $Mensaje = "ALUMNO EXPULSADO";
                                break;
                            case 6:
                                $Mensaje = "ALUMNO EGRESADO";
                                break;
                            case 7:
                                $Mensaje = "ALUMNO BLOQUEADO";
                                break;
                        }
                        if($dato->estatus == '1'){
                            $data[]=array(
                                ...
                                $data[]=array(
                                    0=> $dato->nombre,
                                    1=> $dato->Matricula,
                                    2=> "<div class = 'row  m-0 justify-content-center'><h5> $Mensaje </h5></div>"
                                    );
                            }
                        /*}else{
                            $data[]=array(
            ...
            case 'consultarCalificaciones':
                ...
                $data = Array();
                    while($dato=$obtenerCalificaciones->fetchObject()){
                        $data[]=array(
                        0=> $dato->nombre,
                        1=> '<label class ="d-none">'.$dato->calificacion.'</label><input class="form-control" name = "'.$dato->idCalificacion.'" maxlength="2" onkeyup="checkCalificaciones('.$dato->idalumno.')" id="calificacion'.$dato->idalumno.'" value="'.$dato->calificacion.'">',
        ...
        case 'consultar_clases_materias':
            ...
            $clases = [];
            $vista = $_SESSION['usuario']['estatus_acceso'];
            foreach($materias_car as $key_m => $materia){
                $cls = $matsM->consultar_clases_materia($materia['id_materia'], $_POST['generacion'],$_POST['groupG'],$vista);
                if(!empty($cls)){
        ...
        case 'volcar_alumnos':
            ...
            $_POST['ars'] =  $_SESSION['usuario']['idTipo_Persona'];
            $alumnos = $ce->volcar_alumnos($_POST);
            echo json_encode($alumnos);
        break;
        // case 'consultar_alumnosG':
        //     unset($_POST['action']);
        //     $datas = $ce->consultar_
        //     echo
        // break;
        case 'VerComentariosDirectorio':
            unset($_POST['action']);
            $ComentariosAfiliado = $ce->VerComentariosDirectorio($_POST);
            $data = Array();
            while($dato=$ComentariosAfiliado->fetchObject()){
                $data[]=array(
                    0=> $dato->notas
                );
            }
        ...
        case 'consultar_alumnos_match':
            unset($_POST["action"]);
            $Alumnos = $ce->consultar_alumnos_reservaciones($_POST);
            echo json_encode($Alumnos);
            break;

        case 'consultar_alumnos_generacion':
            unset($_POST['action']);//esta linea eliminba el valor action que se mando por post
            //echo //aqui se almacena los valores que hay en generacion que se mando por post
            $generacion = $_POST['generacion'];
            $acceso =  $_SESSION['usuario']['estatus_acceso'];
            //var_dump($_POST);
            $consultaralumnosgeneraciones = $ce->consultar_alumnos_generacion_listas($generacion,$acceso);//ejecuta esta funcion con el valore 0 que hay en la variable generacion
            echo json_encode($consultaralumnosgeneraciones);//retorna mediante un JSON los valores almacendos
        break;
...
function nombreDocumento( $tipo ){
    switch( $tipo ){
        ...
        case 12: return "Certificado de licenciatura";
        case 13: return "Copia Notariada del Titulo de Medicina";
        case 14: return "Copia Notariada de Diploma de especialidad";
        case 15: return "Cedula profesional anverso";
        case 16: return "Cedula profesional reverso";
        case 17: return "Documento migratorio";
        case 18: return "Dictamen Tecnico";
        case 19: return "Foto Tamaño Titulo";
        case 20: return "Foto Tamaño Credencial";
        case 21: return "Copia Notariada del Titulo de Medicina";
        case 22: return "Copia Notariada de Diploma de especialidad";
        case 23: return "Certificado de licenciatura";
        case 24: return "Cedula profesional";
        default: return "";
    }//fin switch
}//nombreDocumento
...
function enviar_accesos($nombre, $correo){
    date_default_timezone_set("America/Mexico_City");
	...
	for($i = 0; $i < sizeof($destinos); $i++){
        ...
        // file_put_contents('envio.html', $plantilla);
		sendEmailOwn([[ $destinos[$i][1],  $destinos[$i][0]]], 'Envío de accesos', $plantilla);
	}
}
?>
