<?php
	...
	$constancias = $con->query('SELECT * FROM `asistentes_eventos` WHERE `id_evento` = 30')->fetchAll(PDO::FETCH_ASSOC);
	...
	foreach ($constancias as $row => $values) {
		...
		$plantilla = '';
			...
			$plantilla = '../../functions/plantillas_constancias/CURSOLOGOTERAPIAQR.jpg';
		...
		if($persona){
			...
			$nombre_reconocimiento = 'Curso_Logoterapia_'.$nombre.'_'.$values['id_evento'].'_'.$values['id_asistente'];
			// echo "R>>".$nombre."<br>";
		}else{
		...
		$sql = "UPDATE asistentes_eventos set nombre_reconocimiento='$nombre_reconocimiento' WHERE id_asistente='".$values['id_asistente']."' AND id_evento=30";
