...
switch ($accion) {
    ...
    case 'contacto':
        ...
        $rspta2=$afiliados->contactoap($email,$idusuario, $celular);
        break;
    case 'academico':
    ...
    case 'subirFotoPerfil':
        if(isset($_POST['image'])){
            ...
            $name = $idusuario.date('Ymd_Hi') . '.png';
            $image_name = '../../img/afiliados/' . $name;
            var_dump(file_put_contents($image_name, $data));
            $imageP = $afiliados->editarfoto($name, $idusuario);
            echo $image_name;
        }else{
            echo 'no_image';
        }
        ...
        case 'registrar_pago_afiliacion':
            ...
            if ($id_concepto['data']['id_concepto']==3) {
                ...
                $finmembresia = date("Y-m-d H:i:s",strtotime($fecha."+ 6 month"));
                $resgistrarpagosemestral=$afiliados->resgistrarpagosemestral($id_prospecto ,3 ,$detalle,$finmembresia,$monto);
            ...
            if ($id_concepto['data']['id_concepto']==4) {
                ...
                $fecha= $fechafinmembresia['data']['finmembresia'];
                $finmembresia = date("Y-m-d H:i:s",strtotime($fecha."+ 1 year"));
                $resgistrarpagosemestral=$afiliados->resgistrarpagosemestral($id_prospecto ,4 ,$detalle,$finmembresia,$monto);
    ...
    case 'contacto_persona':
        if(isset($_POST['email']) && isset($_POST['firstname']) && isset($_POST['lastname']) && isset($_POST['destino']) && isset($_POST['subject']) && trim($_POST['subject']) != ''){
			...
			$resp = ['estatus'=>'error'];
            $resp['info']=(trim($_POST['subject']) == '')? 'El contenido del mensaje no puede estar vacío':'Faltan datos para enviar el correo';
    ...
    case 'guardar_plantilla':
        $_POST['afiliado'] = $idusuario;
        unset($_POST['action']);
        echo json_encode($afiliados->guardar_plantilla($_POST));
        break;
    
    case 'guardar_asistencia':
        unset($_POST["action"]);
        $numero_clase = $_POST["numClass"];
        $idCarrera="14";
        $idAsistente= $_POST["idAlumno"];
        $fecha = date("Y-m-d H:i:s");

        $ya_tiene_asistencia_hoy = $afiliados->ya_tiene_asistencia($idAsistente,$numero_clase,"14");
        if ($ya_tiene_asistencia_hoy['data']==false) {
            $RegistrarAsistencia = $afiliados->guardar_asistencia("EN LINEA",$numero_clase, $idCarrera, $idAsistente,$fecha);
        }else{
            $RegistrarAsistencia = ["estatus"=>"error", "msj"=>"Ya existia asistencia para este evento"];
        }
        echo json_encode($RegistrarAsistencia);
        break;
...
if(!isset($_SESSION['alumno']) && !isset($_POST['android_id_afiliado']) && !isset($_POST['action'])){
