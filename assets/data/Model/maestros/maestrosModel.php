...
date_default_timezone_set("America/Mexico_City");
	class Maestro{
		...
		public function editarExamen($NombreExamen, $idCurso, $FechaInicio, $FechaFin, $idExamen){
			...
			if($con['info'] == 'ok'){
				...
				if($statement->errorInfo()[0] == "00000"){
					...
					$statement = ['estatus'=>'error', 'info'=>$statement->errorInfo()];
				}
			}
		$conexion = null;
		$con = null;
		...
		public function insertarPreguntaExamen($idExamen, $pregunta, $opciones,$band){
			$conexion = new Conexion();
			$con = $conexion->conectar();
			$response = [];
			...
			if($con['info'] == 'ok'){
				...
				if($band == "validar"){
					$sql = "SELECT * FROM cursos_examen_preguntas
					WHERE  idExamen = :idExam AND pregunta = :question AND opciones = :options";
				}else{
				...
				if($statement->errorInfo()[0] == "00000"){
					$response = ['estatus'=> $band == "validar"? 'validar':'ok' , 'data'=> $band == "validar" ? $statement->rowCount() : $con->lastInsertId()];
				}else{
					$response = ['estatus'=>'error', 'info'=>$statement->errorInfo(), $sql=>'sql'];
				}
			}
		...
		public function insertarPreguntaPasadaExamen($idExamenPas){
			...
			if($con['info'] == 'ok'){
				...
				if($statementPAS->errorInfo()[0] == "00000"){
					...
					$response = ['estatus'=>'error', 'info'=>$statementPAS->errorInfo()];
				}
		...
		public function Rango_de_Preguntas($idExamenPas){
			...
			if($con['info'] == 'ok'){
				...
				$sql = "SELECT * FROM `cursos_examen_preguntas` WHERE idExamen = $idExamenPas GROUP BY pregunta;";
		...
		public function obtenerTareasCalificar($id){
			...
			if($con['info'] == 'ok'){
				$con = $con['conexion'];
				$sql = "SELECT classHom.titulo, classEntregas.idEtrega as idEntrega, classEntregas.comentario, classEntregas.fecha_entrega, classEntregas.calificacion, classEntregas.archivo, CONCAT(aPros.nombre,' ',aPros.aPaterno,' ',aPros.aMaterno) as nombre, classEntregas.retroalimentacion, mat.nombre as NombreMaterias, aGen.nombre NombreGen, mat.nombre NombreMat
					...
					INNER JOIN clases AS class ON class.idClase = classHom.idClase
					INNER JOIN materias AS mat ON mat.id_materia = class.idMateria
					INNER JOIN a_generaciones AS aGen ON aGen.idGeneracion = class.idGeneracion
		...
		public function ActualizarDatosMaestro($Datos){
			...
			if($con['info'] == 'ok'){
				...
				WHERE id = :id;
				UPDATE a_accesos SET correo = :Email WHERE idTipo_Persona = 30 AND idPersona = :id ";
