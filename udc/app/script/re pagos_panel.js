				...
				try{
					...
					if(resp_conceptos.estatus == 'error'){
						...
						for (let j = 0; j < planes.pagos_aplicar.length; j++) {
							...
							var responsePro = solicitar_prorrogaOutClick(planes.idPlan, planes.pagos_aplicar[j].id_concepto, id_carrera, planes.pagos_aplicar[j].aplicados.length + 1,planes.pagos_aplicar[j].fechalimitepago)

							datePro = responsePro[0].response;
							var st = 0;
							if(responsePro[0].status == 'tardy' || responsePro[0].status == 'duplex' || parseInt(planes.pagos_aplicar[j].aplicados.length + 1) > parseInt(planes.pagos_aplicar[j].numero_pagos) ){
							...
							$("#tabla_conceptos_pagar").DataTable().row.add([
								...
								info_promo, (planes.pagos_aplicar[j].categoria=='Mensualidad'||planes.pagos_aplicar[j].categoria=='Titulación')? (monto_fin_promo != 0)?`<button class="btn btn-info btn-sm" onclick="ver_pagos(${planes.idPlan}, ${planes.pagos_aplicar[j].id_concepto}, ${id_carrera},'${datePro}',${st},'${'none'}','${tipomoneda}')">
								Pagar
							</button> `+ `<button class="btn ${tipoboton} btn-sm" onclick="solicitar_prorroga(${planes.idPlan}, ${planes.pagos_aplicar[j].id_concepto}, ${id_carrera}, ${planes.pagos_aplicar[j].aplicados.length + 1}, '${planes.pagos_aplicar[j].fechalimitepago}',${planes.pagos_aplicar[j].numero_pagos})">
							Solicitar prorroga
						</button>`: ' ':(monto_fin_promo != 0)?`<button class="btn btn-info btn-sm" onclick="ver_pagos(${planes.idPlan}, ${planes.pagos_aplicar[j].id_concepto}, ${id_carrera},'${datePro}',${st},'${'none'}','${tipomoneda}')">
							Pagar
						</button> `:''
							]);
					...
					var card_pago = `
						...
						<div class="card text-center mb-4">
							...
							<p class="card-text">${concepto.concepto}</p>
								${string_fecha}
								<h5 class="card-title text-success mb-0">${(pagar.porcentaje_promocion > 0 ? "- "+pagar.porcentaje_promocion.toFixed(2)+"% ":'')}</h5>
		...
		success:function(msj){
			if(msj==1){
				swal({
					...
					text: 'Tu pago se proceso exitosamente, <b>no necesitas reportarlo </b>',
					icon: "success",
				  }).then((value)=>{
					  window.location.reload();
				  })
					...
					if(aplicados[i].estatus == 'verificado'){
						if(aplicados[i].hasOwnProperty('saldo_favor')){
							...
							$("#table_pagos_apli").DataTable().row.add([
								...
								aplicados[i].concepto_nombre + (parseInt(aplicados[i].concepto_numero_pagos) > 1 ? ` <span>[<b>Pago N° ${parseInt(aplicados[i].numero_de_pago)}  de ${parseInt(aplicados[i].concepto_numero_pagos)}</b> ${aplicados[i].restante > 0 ? ' <small>pago parcial</small>':''}]`:''),
								...
								(aplicados[i].cargo_retardo == null || aplicados[i].cargo_retardo == 0)? '$ 0,00': `<span class="text-danger">${moneyFormat.format(aplicados[i].cargo_retardo)} ${aplicados[i].tipomoneda}</span>`,
								...
								(aplicados[i].restante !== null) ? (parseInt(aplicados[i].restante) < 0 ? moneyFormat.format(aplicados[i].restante)+"<br><small class='text-success'> (Saldo a favor)</small>" : moneyFormat.format(aplicados[i].restante))+aplicados[i].tipomoneda : '$ 0,00',
								aplicados[i].saldo,
...
/*$("#metododepago").on('change', function(e){
	var tipoPago = $("#metododepago").val();
	...
	if(tipoPago.trim() != ''){
		switch(tipoPago){
			...
			case 'Transferencia':
				...
				$("#modalVerEjemploPago").modal('show');
				break;
			case 'Pago en ventanilla cuenta general':
				//console.log(3)
				$("#verEjemploComprobante").attr('src','../../assets/images/ejemploComprobantes/'+tipoPago+'.png');
                $("#verEjemploComprobante").show();
				$("#modalVerEjemploPago").modal('show');
			...
			case 'Tarjeta de crédito/débito':
				//console.log(4)
				var nomTipoPago = tipoPago.replace(/\//g, '');
				$("#verEjemploComprobante").attr('src','../../assets/images/ejemploComprobantes/'+nomTipoPago+'.png');
                $("#verEjemploComprobante").show();
				$("#modalVerEjemploPago").modal('show');
				break;
			case 'Pago en efectivo':
				...
				$("#modalVerEjemploPago").modal('show');
				break;
			case 'Pago en cajero automático a cuenta general':
				//console.log(6)
				//swal('Sin ejemplo');
				$("#verEjemploComprobante").attr('src','../../assets/images/ejemploComprobantes/'+tipoPago+'.png');
                $("#verEjemploComprobante").show();
