	...
	$.ajax({
		...
		success: function(data){
			try{
				...
				opc = "<option selected disabled>Seleccione su oferta educativa.</option>";
...
$("#select_inscripciones").on('change', function(){
	...
	if($("#select_inscripciones option:selected").attr('info-e') == 'carrera'){
		$.ajax({
			type: "POST",
			url: "../../assets/data/Controller/planpagos/pagosControl.php",
			data: {action:'obtener_plan_pago_callcenter', inscrito_a : $("#select_inscripciones").val(), instit:20},
			success: function(response) {
				try{
					...
					//resp_conceptos.data.pagos_aplicar.forEach(element => console.log(element));
					...
					if(resp_conceptos.estatus == 'error'){
						...
						for (let j = 0; j < planes.pagos_aplicar.length; j++) {
							...
							// 	tipoboton = 'btn-secondary';
							// }
							//console.log(planes.pagos_aplicar[j].concepto+" "+planes.pagos_aplicar[j].calificacion);
					...
					if(resp_conceptos.estatus == 'error'){
						...
						for (let j = 0; j < planes.pagos_aplicar.length; j++) {
							...
							$("#tabla_conceptos_pagar").DataTable().row.add([
								info_ext+planes.pagos_aplicar[j].concepto+` <small id="aplied_${planes.pagos_aplicar[j].id_concepto}">${conteo}</small>`,
								string_precio,
								info_promo, (planes.pagos_aplicar[j].categoria=='Mensualidad'||planes.pagos_aplicar[j].categoria=='Titulación')? (monto_fin_promo != 0)? `<button class="btn btn-info btn-sm my-2" onclick="ver_pagos(${planes.idPlan}, ${planes.pagos_aplicar[j].id_concepto}, ${id_carrera},'${datePro}',${st})">
								Pagar
							...
							Solicitar prorroga
						</button>`:' ': (monto_fin_promo != 0)?`<button class="btn btn-info btn-sm my-2" onclick="ver_pagos(${planes.idPlan}, ${planes.pagos_aplicar[j].id_concepto}, ${id_carrera},'${datePro}',${st}, 'evento')">
							Pagar
						</button> `:''
							]);
...
let saldo_pendiente_g = false;
let saldo_favor_g = false;
...
function procesar_pago_multi(id_concepto, costo, retardo, num_p, descript, concept, parcialidad, promocion,tipomoneda){
	...
	$("#check_domiciliar_pago").hide();
	if(saldo_pendiente_g){
		if($("#dinam_dom_info").length == 0){
			$("#domiciliar_pago").attr('disabled', true);
			$("#check_domiciliar_pago").append(`<p id="dinam_dom_info" class="card-text"><i class="fa fa-info-circle"></i> <i>No es posible domiciliar sobre un saldo pendiente.</i></p>`)
		}
	}
	if(saldo_favor_g){
		if($("#dinam_dom_info").length == 0){
			$("#domiciliar_pago").attr('disabled', true);
			$("#check_domiciliar_pago").append(`<p id="dinam_dom_info" class="card-text"><i class="fa fa-info-circle"></i> <i>No es posible domiciliar con un saldo a favor aplicado.</i></p>`)
		}
	}
	descripcionpago = ``
	if(retardo > 0){
		...
		if($("#dinam_dom_info").length == 0){
			$("#domiciliar_pago").attr('disabled', true);
			$("#check_domiciliar_pago").append(`<p id="dinam_dom_info" class="card-text"><i class="fa fa-info-circle"></i> <i>No es posible domiciliar si presenta un recargo.</i></p>`)
		}
		//else{
		// 	$("#domiciliar_pago").attr('disabled', false);
		// 	$("#dinam_dom_info").remove();
		// }
	}else{
		if($("#dinam_dom_info").length == 0 && !saldo_pendiente_g && !saldo_favor_g){
			$("#domiciliar_pago").attr('disabled', false);
			$("#dinam_dom_info").remove();
		}
	}
...
$("#domiciliar_pago").on('change', function(){
	if($("#domiciliar_pago").prop('checked')){

		swal({
			title:'Atención',
			text:'¿Seguro que desea domiciliar los pagos de sus mensualidades?',
			buttons:['Cancelar','Aceptar']
		}).then(sel => {
			if(!sel){
				$("#domiciliar_pago").attr('checked', false);
			}
		})
	}
})
					...
					if(aplicados[i].estatus == 'verificado'){
						if(aplicados[i].hasOwnProperty('saldo_favor')){
							...
							$("#table_pagos_apli").DataTable().row.add([
								...
								aplicados[i].concepto_nombre + (parseInt(aplicados[i].concepto_numero_pagos) > 1 ? ` <span>[<b>${aplicados[i].concepto_categoria == 'Mensualidad' ? 'Mensualidad' : 'Pago'} ${parseInt(aplicados[i].numero_de_pago)}</b> ${aplicados[i].restante > 0 ? ' <small>pago parcial</small>':''}]`:''),
								...
								`<span class="d-block font-weight-bold">${moneyFormat.format(parseFloat(aplicados[i].detalle_pago.purchase_units[0].amount.value) )} ${aplicados[i].tipomoneda}</span>`,
								`
								<b class="${(real_restante > 0.9 ) ? 'text-danger' :''}">Concepto: </b> ${moneyFormat.format(real_restante)+' '+aplicados[i].tipomoneda} <br> 
								<b class="${(aplicados[i].saldo > 0.9 ) ? 'text-danger' :''}">Recargo: </b> ${moneyFormat.format(aplicados[i].saldo)+' '+aplicados[i].tipomoneda}
								...
								`,
								((aplicados[i].numero_de_pago == aplicados[i].max_aplicado) || (parseInt(aplicados[i].concepto_numero_pagos) == 1 && (aplicados[i].restante > 0 || aplicados[i].saldo > 0)) ) ? `<button class="btn btn-info btn-sm my-2" onclick="ira_pagos(${aplicados[i].id_concepto}, ${aplicados[i].carrera_id},'${aplicados[i].tipomoneda}')">Pagar</button>` : ''
							]);
						}
...
$( "#metododepago1" ).change(function() {
  ...
  function consultar_domiciliacion(){
	if(plan_pago.hasOwnProperty('generaciones') && plan_pago.generaciones.length > 0){
		var gen_asign = plan_pago.generaciones.find(elm => elm.asignacion.length > 0);
		if(gen_asign){
			$.ajax({
				url: '../../assets/data/Controller/planpagos/domiciliacionControl.php',
				type: 'POST',
				data: {action: 'conultar_domiciliacion', generacion: gen_asign.idGeneracion},
				dataType: "JSON",
				success: (e)=>{
					if(e.estatus == 'ok'){
						if(e.data.plan_domiciliacion != '' && e.data.plan_domiciliacion != null){
							$("#domiciliar_pago").attr('disabled', true);
							$("#check_domiciliar_pago").append(`<p id="dinam_dom_info" class="card-text"><i class="fa fa-info-circle"></i> <i>Este concepto ya se encuentra domiciliado.</i></p>`)
						}else{
							$("#domiciliar_pago").attr('disabled', false);
							$("#dinam_dom_info").remove();
						}
