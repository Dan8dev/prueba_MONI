...
class Generaciones{
	public function validarBloqueo($idAlumno, $idGeneracion){
		...
		if($con["info"] == "ok"){
			...
			$sql = "SELECT *  FROM alumnos_generaciones AS alumGen WHERE alumGen.idalumno = :idAlum AND alumGen.idgeneracion = :idGene;";

			$statement = $con->prepare($sql);
			$statement->bindParam(':idAlum', $idAlumno);
			$statement->bindParam(':idGene', $idGeneracion);
			$statement->execute();
	...
	public function buscarCarrerasG($busC){
		...
		if($con['info'] == 'ok'){
			...
			$sql = "SELECT idCarrera FROM `a_generaciones` WHERE idGeneracion = :cap";
			...
			if($statement->errorInfo()[0] == "00000"){
				...
				$response = ["estatus"=>"error", "info"=>$statement->errorInfo(), "sql"=>$sql, "data"=>$busC];
			}
	...
	function obtenerFechaLimitePago($alumno, $concepto){
		$conexion = new Conexion();
		$con = $conexion->conectar()['conexion'];
		return $con->query("SELECT max(fecha_limite_pago) as fecha_limite_pago FROM `a_pagos` WHERE `id_prospecto` = {$alumno} AND `id_concepto` = {$concepto};")->fetch(PDO::FETCH_ASSOC);
	}
...
public function validarBecaAlumno($idProspecto, $idGeneracion){
	...
	public function validar_primer_pago($idGeneracion){
		$conexion = new Conexion();
		$con = $conexion->conectar();
		$response = [];
		...
		if($con['info'] == 'ok'){
			...
			$sql = "SELECT * 
					FROM fechas_ciclos 
					WHERE id_generacion = :id_generacion AND ciclo = 2";
