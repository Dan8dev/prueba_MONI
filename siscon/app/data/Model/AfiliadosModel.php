    ...
    class Afiliados
    {
		...
		public function InsertarSolicitudCred($data){
			...
			if($con['info'] == 'ok'){
				...
				VALUES ( :idusuario, :idCur, NOW(), NULL, 1,0);";
		...
		public function obtenerSolicitudesCredencial($idAlumno){
			...
			if($con['info'] == 'ok'){
				$con = $con['conexion'];
				$sql = "SELECT * FROM solicitud_credenciales WHERE idalumno = :idAlum and institucion = 0 ORDER BY idsolicitud DESC LIMIT 1;";
		...
		public function fechafinGeneracion($id_prospecto){
			//var_dump($id_prospecto);
			$conexion = new Conexion();
			$con = $conexion->conectar();
			$response = [];
			...
			if($con["info"] == "ok"){
				...
				$sql = "	SELECT gen.*, carr.nombre as nomCarrer, plE.nombre as nombrePlan, plE.numero_ciclos
                	...
                	WHERE gen.estatus = 1 AND gen.iDGeneracion = :id_prospecto;";
				$statement = $con->prepare($sql);
				$statement->bindParam(":id_prospecto", $id_prospecto);
				$statement->execute();
		...
		public function obtenerVistas(){
			...
			if($con['info'] == 'ok'){
				...
				$sql = "SELECT idVista, nombre, directorio, icono, publica, menu FROM vistas WHERE estatus = 1 AND icono != ''";
		...
		public function cunsultar_si_generacion_institucion($prospecto, $institucion){
			...
			$con = $conexion->conectar();
			$con = $con['conexion'];
			$inscrito = $con->query("SELECT ag.*, cr.idInstitucion FROM alumnos_generaciones ag 
			JOIN a_generaciones gen ON gen.idGeneracion = ag.idgeneracion
			JOIN a_carreras cr ON cr.idCarrera = gen.idCarrera
			WHERE ag.estatus NOT IN (2, 5) AND ag.idalumno = ".$prospecto." AND cr.idInstitucion = ".$institucion." ;")->rowCount();
			return $inscrito;
		}

		public function consultar_pacientes_encargado_evento($encargado, $evento){
			$conexion = new Conexion();
			$con = $conexion->conectar();
			$con = $con['conexion'];
			$sql = "SELECT aten.*, prs.nombre, prs.aPaterno, prs.aMaterno FROM `a_marketing_atencion` aten
				JOIN a_prospectos prs ON prs.idAsistente = aten.prospecto
				WHERE prs.idAsociacion = (SELECT pp.idAsociacion FROM a_prospectos pp WHERE pp.idAsistente = :encargado)
				AND aten.tipo_atencion = 'evento' AND aten.evento_carrera = :evento;";
			$stmt = $con->prepare($sql);
		...
		function consultar_registro_evento($alumno, $evento){
			$conexion = new Conexion();
			$con = $conexion->conectar();
			$con = $con['conexion'];
			$sql = "SELECT * FROM a_marketing_atencion WHERE prospecto = :prospecto AND evento_carrera = :evento AND tipo_atencion = 'evento';";
			$stmt = $con->prepare($sql);
			$stmt->bindParam(':prospecto', $alumno);
			$stmt->bindParam(':evento', $evento);
			$stmt->execute();
