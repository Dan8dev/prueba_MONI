...
class Promociones
{
    ...
    public function crearPromocionalumno_numeroPago($nombre, $concepto, $tipo, $prospecto, $porcentaje, $creado_por, $fechacreado, $estatus, $numero_aplicar, $id_of = null)
    {
        $conexion = new Conexion();
        $con = $conexion->conectar();
        $con = $con['conexion'];
        $response = [];
        $sql = "INSERT INTO promociones (nombrePromocion, tipo, id_concepto, id_prospecto, porcentaje, creado_por,fechacreado, estatus, Nopago, id_oferta )
		  VALUES (:nombrepromocion, :tipo, :id_concepto, :id_prospecto,:porcentaje,:creado_por,:fechacreado, :estatus, :pagos_aplicar, :id_of); ";
        $statement = $con->prepare($sql);
        ...
        $statement->bindParam(':id_of', $id_of);
    ...
    public function crearPromociongeneracion_numeroPago($nombre, $concepto, $tipo, $generacion, $porcentaje, $creado_por, $fechacreado, $estatus, $numero_aplicar)
    {
        $conexion = new Conexion();
        $con = $conexion->conectar();
        $con = $con['conexion'];
    ...
    public function crearPromocionalumno($nombre, $id_concepto, $tipopromobeca, $id_prospecto, $porcentajepromocion, $creado_por, $fechacreado, $estatus, $fechainicio, $fechafin, $id_of)
    {
        $conexion = new Conexion();
        $con = $conexion->conectar();
        $response = [];
        ...
        if ($con["info"] == "ok") {
            $con = $con["conexion"];
            $sql = "INSERT INTO promociones (nombrePromocion, tipo, id_concepto, id_prospecto, porcentaje, creado_por,fechacreado, estatus, fechainicio, fechafin, id_oferta)
		  VALUES (:nombrepromocion, :tipo, :id_concepto, :id_prospecto,:porcentaje,:creado_por,:fechacreado, :estatus, :fechainicio, :fechafin, :id_of); ";
    ...
    public function obtenerPromocion($idpromocion)
    {
        ...
        if ($con['info'] == 'ok') {
            $con = $con['conexion'];
            $sql = "SELECT promo.idPromocion,promo.nombrePromocion,promo.tipo, pagcon.id_concepto as id_concepto, pagcon.precio, agen.idGeneracion as id_generacion,ac.idCarrera as id_carrera, promo.porcentaje, promo.Nopago, promo.fechacreado, ac.nombre as nombrecarrera, agen.nombre as nombregeneracion, pagcon.descripcion as nombreconcepto, promo.fechainicio, promo.fechafin, aficon.idAsistente as id_prospecto, pagcon.categoria, pagcon.numero_pagos
				FROM promociones as promo
				left JOIN pagos_conceptos as pagcon on pagcon.id_concepto=promo.id_concepto
				LEFT JOIN a_generaciones as agen on agen.idGeneracion=promo.id_generacion
				LEFT JOIN a_carreras as ac on ac.idCarrera=promo.id_carrera
            ...
            if ($statement->errorInfo()[0] == '00000') {
                $dta = $statement->fetch(PDO::FETCH_ASSOC);
                if ($dta) {
                    $dta['Nopago'] = json_decode($dta['Nopago']);
                }
                $response = ["estatus" => "ok", "data" => $dta];
            } else {
                $response = ["estatus" => "error", "info" => $statement->errorInfo(), "sql" => $sql];
            }
            $conexion = null;
    ...
    // agrego chuy
    public function obtenerPromocion_concepto_alumno($alumno, $concepto)
    {
        $conexion = new Conexion();
        $con = $conexion->conectar();
        $response = [];
        ...
        if ($con['info'] == 'ok') {
            $con = $con['conexion'];
            $sql = "SELECT promo.* FROM promociones as promo
				WHERE promo.estatus = 'activo' AND promo.id_prospecto = :alumno AND promo.id_concepto = :concepto";
            $statement = $con->prepare($sql);
            ...
            if ($statement->errorInfo()[0] == '00000') {
                $data = $statement->fetchAll(PDO::FETCH_ASSOC);
                foreach ($data as $promo => $value) {
                    if ($value['Nopago'] != null && $value['Nopago'] != '') {
                        $num_p = json_decode($value['Nopago'], true);
                        if ($num_p) {
                            $data[$promo]['Nopago'] = $num_p;
                        }
                    }
                }
                $response = ["estatus" => "ok", "data" => $data];
    ...
    public function getConceptosGenracion($id_generacion)
    {
        ...
        if ($con['info'] == 'ok') {
            $con = $con['conexion'];
            $sql = "SELECT pc.id_concepto,pc.concepto, pc.descripcion, pc.precio,pc.precio_usd, pc.numero_pagos
			FROM pagos_conceptos as pc
			JOIN a_generaciones as ag on ag.idGeneracion =pc.id_generacion
			WHERE ag.idGeneracion=:id_generacion AND pc.categoria IN ('Inscripción', 'Reinscripción', 'Titulación', 'Mensualidad');";
    ...
    public function getConceptosplan($id_generacion)
    {
        ...
        if ($con['info'] == 'ok') {
            $con = $con['conexion'];
            $sql = "SELECT pc.id_concepto,pc.concepto, pc.descripcion, pc.precio, pc.precio_usd
			FROM pagos_conceptos as pc
    ...
    public function activarpromocion($data)
    {
        $conexion = new Conexion();
        $con = $conexion->conectar();
        $response = [];
    ...
    public function desactivarpromocion($data)
    {
        $conexion = new Conexion();
        $con = $conexion->conectar();
        $response = [];
    ...
    public function actualizarpromocion($idpromocion, $nuevonombre, $tipopromoobeca, $procentajepromocion, $actualizadopor, $fechaactualizado, $fechainicio, $fechafin)
    {
        $conexion = new Conexion();
        $con = $conexion->conectar();
        $response = [];
        ...
        if ($con['info'] == 'ok') {
            $con = $con['conexion'];
            $sql = "UPDATE promociones SET nombrePromocion = :nombrepromocion, tipo = :tipo, porcentaje = :porcentaje, actualizado_por = :actualizado_por, fecha_actualizado = :f_actualizacion, fechainicio = :fechainicio, fechafin = :fechafin
    ...
    public function actualizar_Nopago_pagos_promocion($promo, $numero_pagos)
    {
        $conexion = new Conexion();
        $con = $conexion->conectar()['conexion'];
        $sql = "UPDATE promociones SET Nopago = :numero_pagos WHERE idPromocion = :idPromocion";
        $statement = $con->prepare($sql);
        $statement->bindParam(":idPromocion", $promo);
    ...
    public function crear_oferta($data)
    {
        $conexion = new Conexion();
        $con = $conexion->conectar()['conexion'];
        $response = [];
    ...
    public function consultar_ofertas_generacion($generacion)
    {
        $conexion = new Conexion();
        $con = $conexion->conectar()['conexion'];
	...
	function consultar_ofertas($estatus = 1){
		$conexion = new Conexion();
        $con = $conexion->conectar()['conexion'];
        
		$sql = "SELECT pof.*, ag.secuencia_generacion as numero_generacion, crs.nombre as nombre_carrera FROM promo_ofertas pof
		JOIN a_generaciones ag ON ag.idGeneracion = pof.generacion
		JOIN a_carreras crs ON crs.idCarrera = ag.idCarrera
		WHERE pof.estatus = :estatus;";
	...
	function consultar_promos_aplicadas_oferta($oferta){
		$conexion = new Conexion();
        $con = $conexion->conectar()['conexion'];
        
		$sql = "SELECT prm.*, prs.aPaterno, prs.aMaterno, prs.nombre FROM promociones prm 
			JOIN a_prospectos prs ON prs.idAsistente = prm.id_prospecto
			WHERE prm.id_oferta = :oferta GROUP BY prm.id_prospecto;";
		$stmt = $con->prepare($sql);
