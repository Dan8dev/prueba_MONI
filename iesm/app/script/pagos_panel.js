...
$("#select_inscripciones").on('change', function(){
	...
	if($("#select_inscripciones option:selected").attr('info-e') == 'carrera'){
		$.ajax({
			type: "POST",
			url: "../../assets/data/Controller/planpagos/pagosControl.php",
			data: {action:'obtener_plan_pago_callcenter', inscrito_a : $("#select_inscripciones").val(), instit:19},
				...
				try{
					...
					if(resp_conceptos.estatus == 'error'){
						...
						for (let j = 0; j < planes.pagos_aplicar.length; j++) {
							...
							var	normal = true;
							var examen = false;
							if(planes.pagos_aplicar[j].concepto.includes("(") && planes.pagos_aplicar[j].concepto.includes(")")){
								examen = true;
								normal = false;
							}
							...
							var Reprobado = false;
							if(examen){
								if(parseInt(planes.pagos_aplicar[j].calificacion) < parseInt(planes.pagos_aplicar[j].calificacion_min)){
									Reprobado = true;
