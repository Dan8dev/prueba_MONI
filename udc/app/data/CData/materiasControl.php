...
switch ($_POST['action']) {
    ...
    case 'terminar_examen':
        ...
        case 'consultar_calificaciones':
            ...
            break;
        case 'obtenerDatosPago':
            unset($_POST['action']);
            $fechaHoy = date('Y-m-d');
            $pagos = 0;
            //("Y-m-d", strtotime($obPlanE['fecha_inicio']))
            $obConceptos = $generacionesM->obtenerConceptosPago($_POST['id'])['data'];
            
            for($z = 0 ; $z < count($obConceptos); $z++){
                if($obConceptos[$z]['categoria'] == "Inscripción"){
                    $idInscripcion = $obConceptos[$z]['id_concepto'];
                }
                if($obConceptos[$z]['categoria'] == "Mensualidad"){
                    $idMensualidad = $obConceptos[$z]['id_concepto'];
                    $fechaMensualidad = $obConceptos[$z]['fechalimitepago'];
                }
            }
            //var_dump($idMensualidad);
            $obPagoInscripcion = $generacionesM->obtenerInscripcionAlumno($_POST['idAlumno'], $idInscripcion)['data'];
            $obPagoMensualidades = $generacionesM->obtenerMensualidadesAlumno($_POST['idAlumno'], $idMensualidad)['data'];
                            
            if($obPagoInscripcion != []){
                $obfechaInicioEspecial = $generacionesM->obtenerFechaAlumnoEspecial($_POST['idAlumno'], $_POST['id'])['data'];
                if($obfechaInicioEspecial["fecha_primer_colegiatura"] != []){
                    // obtener la diferencia de fechas
                    $fechaIni = new Datetime($obfechaInicioEspecial['fecha_primer_colegiatura']);
                    $fechaFin = new Datetime($fechaHoy); 
                    $diferencia = $fechaIni->diff($fechaFin);
                    $difmes =  $diferencia->format('%m');
                    $difdia = $diferencia->format('%d');

                    if($difmes == 0){
                        if($obfechaInicioEspecial['fecha_primer_colegiatura'] <= $fechaHoy){
                            $pagos++;
                            if($pagos == count($obPagoMensualidades)){
                                echo 'si';
                            }
                        }
                    }else{
                        $pagos = $difmes;
                        $diaInicio = date("d", strtotime($obfechaInicioEspecial['fecha_primer_colegiatura']));
                        $diaFin = date("d", strtotime($fechaHoy));
                        if($diaInicio <= $diaFin){
                            $pagos++;
                            if($pagos == count($obPagoMensualidades)){
                                echo 'si';
                    ...
                    if($obPagoMensualidades != []){
                        $obFechaGeneracion = $generacionesM->obtenerFechaGeneracion($_POST['id'])['data'];
                        //var_dump($obFechaGeneracion);
                        //var_dump($fechaMensualidad);
                        $fecha_lim = substr($obFechaGeneracion['fecha_inicio'],0,8).explode('-', $fechaMensualidad)[2];
                        //var_dump($fecha_lim);

                        $fechaIni = new Datetime($fecha_lim);
                        $fechaFin = new Datetime($fechaHoy); 
                        $diferencia = $fechaIni->diff($fechaFin);
                        $difmes =  $diferencia->format('%m');
                        $difdia = $diferencia->format('%d');
                        if($difmes == 0){
                            if($fecha_lim <= $fechaHoy){
                                $pagos++;
                                if($pagos == count($obPagoMensualidades)){
                                    echo 'si';
                                }
                            ...
                            $pagos = $difmes;
                            $diaInicio = date("d", strtotime($fecha_lim));
                            $diaFin = date("d", strtotime($fechaHoy));
                            if($diaInicio <= $diaFin){
                                $pagos++;
                                if($pagos == count($obPagoMensualidades)){
                        ...
                        $compare = null;
                        if($obfechaInicioEspecial["fecha_primer_colegiatura"] != []){
                            $compare = strtotime($obfechaInicioEspecial['fecha_primer_colegiatura']);
                        }else{
                            $obFechaGeneracion = $generacionesM->obtenerFechaGeneracion($_POST['id'])['data'];
                            $compare = strtotime(substr($obFechaGeneracion['fecha_inicio'],0,8).explode('-', $fechaMensualidad)[2]);
                        }
                        $c_hoy = strtotime(date("Y-m-d"));
            ...
            case 'validar_adeudos':
                ...
                $id_afiliado_c = isset($_POST['android_id_afiliado']) ? $_POST['android_id_afiliado'] : $_SESSION['alumno']['id_afiliado'];
                unset($_POST['Nacion']);
                ...
                if($Bloqueosc['data']['estatus']=='2' || $Bloqueosc['data']['estatus']=='7'){
                    ...
                    $adeudo_i = true; // LOS ADEUDOS SE INICIALIZAN EN TRUE Y CAMBIAN A DALSE CUANDO SE COMPRUEBA EL ESTATUS DEL PAGO
                    $adeudo_m = true;
                    $adeudo_doc = false;
                    $adeudo_doc_f = false;
                    $alumno_becado=false;
                    ...
                    foreach ($obConceptos as $key => $value) {
                        if($value['categoria'] == 'Inscripción'){
                            ...
                            foreach($pagado as $pago){
                                if($pago['promesa_de_pago'] != null || $pago['restante'] <= 0){
                                    $adeudo_i = false;
                                }
                                $promo_cien = false;
                        ...
                        if($value['categoria'] == 'Mensualidad'){
                            ...
                            foreach($pagado as $pago){
                                ...
                                if($pago['numero_de_pago'] > $num_p && ($pago['restante']+$pago['saldo']) < 1){
                                    $num_p = $pago['numero_de_pago'];
                                }
                            }
                            $pagos_aplicados = sizeof($pagado);
                            $f_ini = substr($f_ini,0,10);
                            if($pagos_aplicados > 0){
                                $f_ini = date('Y-m-d', strtotime("+{$num_p} month", strtotime($f_ini)));
                            }
                            // verificar si tiene una prorroga para el numero de pago actual
                            $consultar_p = $pagosM->validar_si_existe_prorroga($alumno, $value['id_concepto'], $num_p + 1);
                    ...
                    foreach($estat_documentos as $key => $value){
                        if($value['fecha_digital'] != null){
                            if(strtotime($fecha_hoy) > strtotime($value['fecha_digital'])){
                                ...
                                $prorroga = $controlEscM->validar_prorroga_documento_alumno($id_afiliado_c, $value['id_documento']);
                                if($prorroga && $prorroga['fecha_prorroga_digital'] != null){
                                    if(strtotime($fecha_hoy) > strtotime($prorroga['fecha_prorroga_digital'])){
                                        ...
                                        $entrega = $controlEscM->validar_documento_alumno($id_afiliado_c, $value['id_documento']);
                                        
                                        //Aqui puede entrar la validacion para los alumnos nacionales e internacionales
                                        if($nacionalidad == "Mexicano"){
											if($value['nacionalidad'] == 0 || $value['nacionalidad'] == 1){
												if(!$entrega || $entrega['validacion'] != 1){
													$adeudo_doc = true;
												}
											}
                                        }else{
											if($value['nacionalidad'] == 0 || $value['nacionalidad'] == 2){
												if(!$entrega || $entrega['validacion'] != 1){
													$adeudo_doc = true;
												}
											}
                            ...
                            if(strtotime($fecha_hoy) > strtotime($value['fecha_fisico'])){
                                ...
                                if($prorroga && $prorroga['fecha_prorroga_fisica'] != null){
                                    if(strtotime($fecha_hoy) > strtotime($prorroga['fecha_prorroga_fisica'])){
                                        // consultar si ya hizo entrega del documento
                                        $entrega = $controlEscM->validar_documento_fisico_alumno($id_afiliado_c, $value['id_documento']);
                    ...
                    if((!$adeudo_i && !$adeudo_m && !$adeudo_doc  && !$adeudo_doc_f) || (!$adeudo_doc  && !$adeudo_doc_f && $alumno_becado)){
                        if(isset($_POST['android_id_prospecto'])){
