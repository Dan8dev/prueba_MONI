  ...
  $.ajax({
    ...
    success: function (data) {
      try {
        ...
        $("#select-clase").val(
          list_clases.length > 0 ? list_clases[0].idClase : ""
        );
        setTimeout(function () {
          $("#select-clase").change();
        }, 100);
...
function cargar_cursos_pagos() {
  // consultar carreras con generaciones asignadas al alumno
  $.ajax({
    url: "data/CData/materiasControl.php",
    type: "POST",
      ...
      try {
        ...
        if (cursos_disp.estatus == "ok") {
          ...
          for (i = 0; i < cursos_disp.data.length; i++) {
            var foto_g = "";
            console.log(cursos_disp.data[i].idInstitucion);
            if (cursos_disp.data[i].idInstitucion == 13) {
              if (cursos_disp.data[i].imagen_generacion != "") {
                ...
                xhr.send();
                if (xhr.status == "404") {
                  foto_g = "../../assets/images/generales/flyers/default.png";
                }
              } else {
                foto_g = `../../assets/images/generales/flyers/default.png`;
              }
              ...
              cardsVisibles(
                cursos_disp.data[i].idalumno,
                cursos_disp.data[i].idgeneracion,
                cursos_disp.data[i].nombre,
                nombre_gen,
                foto_g
              );
            }
...
$("#select-clase").on("change", function () {
  clase = list_clases.find(
    (elm) => elm.idClase == $("#select-clase option:selected").val()
  );
  ...
  if (clase) {
    ...
    $("#recurso_descargable").html(
      lis != "" ? lis : "<li>No hay recursos disponibles</li>"
    );
    ...
    $("#material_apoyo").html(
      lis != "" ? lis : "<li>No hay material de apoyo disponible</li>"
    );
    ...
    $("#content_tareas").html(
      hwks != "" ? hwks : "Aún no hay tareas para esta clase"
    );

    if (clase.foto.trim() != "") {
      $("#image-clase").attr(
        "src",
        "https://moni.com.mx/assets/files/clases/fotoClase/" + clase.foto
      ); // <- solo para el panel de siscon, que está en otro dominio
    }
  } else {
  ...
  for (i = 0; i < tarea.entregas.length; i++) {
    ...
    html_t += `<div class="row py-3 border border-top-0 border-right-0 border-left-0 border-secondary">
                    <div class="col-sm-6 ml-auto">
                      <p class="card-text mb-1">Entregado ${
                        t.fecha_entrega.split(" ")[0]
                      } <i>${
      t.fecha_entrega.split(" ")[1]
    }</i> <a href="#"><i class="fa fa-file"></i></a></p>
                      <span>${t.comentario}</span>
                    </div>
                    <div class="col-sm-6 ml-auto pt-1">
                        <span>Calificación: ${
                          t.calificacion === null ? "pendiente" : t.calificacion
                        }</span>
                      <div class="progress">
      ...
      try {
        ...
        if (resp.estatus == "ok") {
          swal({
            icon: "success",
            title: "Tarea enviada",
            text: "espere calificación",
          });
        } else {
          if (resp.info == "error_al_adjuntar_tarea") {
            swal({
              icon: "info",
              text: "Ha ocurrido un error al adjuntar el archivo. Verifique los requisitos.",
            });
          } else {
            swal({
              icon: "info",
              text: "Ha ocurrido un error al entregar su tarea" + resp.info,
            });
          }
        }
        ...
        if (materias.estatus == "ok") {
          if (materias.data[0].materias_ciclo.length > 0) {
            ...
            if (materias.data[0].materias_ciclo.length > 0) {
              ...
              $("#lbl_nombre-mat").html(
                $(".nav-tabs_i.card-header-tabs a")[0].text.trim()
              );
            }
    ...
    success: function (data) {
      try {
        ...
        if (examenes_r.length == 0) {
          html_ex = `Aún no hay exámenes para este curso`;
        }
          ...
          if (examenes_r[i].presentaciones.length > 0) {
            ...
            if (examenes_r[i].multiple_intento == 1) {
              ...
              for (x in examenes_r[i].presentaciones) {
                if (
                  parseInt(examenes_r[i].presentaciones[x].calificacion) >=
                  parseInt(examenes_r[i].porcentaje_aprobar)
                ) {
                  aplicar_nuevamente = false;
                }
              }
  ...
  $.ajax({
    ...
    success: function (data) {
      try {
        ...
        var html_sesiones = ``;
        for (i in sesiones) {
          html_sesiones += `<p><a style="text-decoration: underline;" class="text-primary" href="claseswebex/?sesion=${sesiones[i].id}"><h3>Clic para la sesión de <b>${sesiones[i].nombre_clase} </b> (${sesiones[i].fecha_clase})</h3></a></p>`;
        }
  ...
  $.ajax({
    ...
    success: function (data) {
      try {
        ...
        var periodo = "Cuatrimestre";
        switch (data[0].tipoCiclo) {
          case "1":
        ...
        $.each(data, function (i, item) {
          var semestre = i + 1;
          $("#mostrar-periodos").append(
            "<table class='table table-colored table-dark' > <thead><th colspan='2' class='text-center'> " +
              periodo +
              ": " +
              semestre +
              "</th></thead> <tr><th  class='wd-10p text-center' id='materias" +
              i +
              "'>Materias</th><th class='wd-10p text-center' id='calificacion" +
              i +
              "'>Calificaciones</th></tr> <tbody></tbody></table>"
          );

          // $('#calificacion'+i).append("<th class='wd-10p'>Cuatrimestre "+ semestre +"</th>")
          // $.each(data[i].materias_calificadas, function(k, item3) {
          // $('#calificacion'+i).append("<td style='white-space: break-spaces;' class='wd-10p'>"+data[i].materias_calificadas[k].calificacion+"</td></tr>")
          // });
          ...
          $.each(data[i].materias_calificadas, function (j, item2) {
            $("#materias" + i).append(
              "<tr style='white-space: break-spaces;'  class='wd-10p'><th>" +
                data[i].materias_calificadas[j].nombre +
                "</th> </tr>"
            );
            $("#calificacion" + i).append(
              "<tr style='white-space: break-spaces;'  class='wd-10p'><td style='white-space: break-spaces;'  class='wd-10p text-center'>&nbsp;&nbsp;&nbsp;&nbsp;" +
                data[i].materias_calificadas[j].calificacion +
                "</td></tr>"
            );
          });
        });
...
function cardsVisiblesUDC(idAlumno, id, nombreCarrera, nombreGeneracion, foto) {
  ...
  console.log(Nacionalidad + " hey");
  $.ajax({
    url: "data/CData/materiasControl.php",
    type: "POST",
    data: {
      action: "validar_adeudos",
      id: id,
      idAlumno: idAlumno,
      Nacion: Nacionalidad,
    },
    // complete:function(e)=>{};
    success: function (data) {
      console.log(id, nombreGeneracion);
      if (data.trim() == "si" || id == 1) {
        html_c = `<div class="col-sm-6 col-md-6 col-xl-4">
						<div class="card mg-b-40">
							<div class="card-body bg-blue "  style="column-count: 2; background-color: #EDEDED  ">
							<img class="card-img-bottom img-fluid" src="${foto}" alt="Image"> 
							
							<div class="texto style="color: #484848">
							<p class="card-text text-truncate  mb-0 "   >${nombreCarrera}</p>
							<small class="card-text text-truncate  text-sm">${nombreGeneracion}</small><br>
							<small class="card-text text-truncate  text-sm">Aqui va la hora de la clase</small>
							
							</div>

							
							</div>
							<a href="clases.php?curso=${id}"></a>
						</div>
					</div>`;
        if (id == 85) {
          html_c += `<div class="col-sm-6 col-md-6 col-xl-4">
						<div class="card mg-b-40">
							...
							<p class="card-text text-truncate text-white mb-0">ATENTO AVISO</p>
								<small class="card-text text-truncate text-white text-sm"></small>
							</div>
							<a href="#"><img class="card-img-bottom img-fluid" src="https://
moni.com.mx/assets/images/portadamoni.png" alt="Image"></a>
						</div>
					</div>`;
        }
        ...
        switch (data.trim()) {
          case "no inscripcion":
            mensaje =
              "Su acceso se ha bloqueado por no contar con registro de pago de inscripción";
            break;
          case "no mensualidad":
            mensaje =
              "Su acceso se ha bloqueado por no contar con registro de pago de mensualidad";
            break;
          case "no documentos":
            mensaje =
              "Su acceso se ha bloqueado falta de entrega de documentos digitales";
            break;
          case "no documentos fisicos":
            mensaje =
              "Su acceso se ha bloqueado falta de entrega de documentos físicos";
            break;
          case "baja":
            mensaje = "";
            break;
          case "Alumno_con_baja":
            mensaje = `<small>${nombreCarrera}</small><br>Su acceso se ha bloqueado por baja para este curso`;
            break;
          case "Alumno_con_bloqueo":
            mensaje = `<small>${nombreCarrera}</small><br>Su acceso se ha bloqueado para este curso, contacte a control escolar`;
            break;
        }
        if (mensaje != "") {
          $("#cursos-container")
            .append(`<div class="alert alert-bordered alert-warning text-dark" role="alert" style="max-height: 100px;">
						<strong class="d-block d-sm-inline-block-force"></strong> ${mensaje}<br>.
					</div>`);
        }
      }
    },
