...
function agregarTareaForm( $idMaestro ){
	...
	WHERE clases.idMateria = a_carreras.idCarrera;";
...
function editarTarea( $idMaestro ){
	$con = conect();
	$sql = "UPDATE clases_tareas SET idClase = '".$_POST['idClase']."', titulo = '".$_POST['nombre']."' , descripcion = '".$_POST['descripcion']."', archivo = '".$_POST['recurso']."', idMaestro = $idMaestro WHERE idTareas = ".$_GET['idTarea'];
	$con->query($sql);
...
function agregarTarea( $idMaestro ){
	...
	(idClase, titulo, descripcion, archivo, idMaestro) 
	 VALUES 
	 ('".$_POST['idClase']."', '".$_POST['nombre']."', '".$_POST['descripcion']."', '".$_POST['recurso']."', $idMaestro )";
	 $con->query($sql);
	// echo $sql;
...
function agregarClase( $idMaestro ){
	...
	(idMateria, titulo) 
	 VALUES 
	 ('".$_POST['idCurso']."', '".$_POST['nombreClase']."' )";
	 $con->query($sql);
	 //echo $sql;
	header( "Location: index.php?e=1" );
...
function listarTareasMaestros( $idMaestro ){
	$con = conect();
	$sql = "SELECT clases.idClase AS idClase, clases.idMateria AS idCurso, clases.titulo AS nombreClase, clases_tareas.idTareas AS idTareas, clases_tareas.titulo AS tituloTarea, a_carreras.nombre AS nombreCurso, clases_tareas.descripcion 
	FROM clases, clases_tareas, a_carreras 
	WHERE clases.idClase = clases_tareas.idClase AND idMaestro = $idMaestro AND a_carreras.idCarrera = clases.idMateria;";
	$resultado = $con->query($sql);
	return $resultado;
