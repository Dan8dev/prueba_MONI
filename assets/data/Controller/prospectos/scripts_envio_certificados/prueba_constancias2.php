<?php
	...
	foreach ($constancias as $row => $values) {
		...
		if($values['id_taller'] == 1){
			$plantilla = '../../functions/plantillas_constancias/tallertratamientodelasprincipales.jpg';
		}else if($values['id_taller'] == 2){
			$plantilla = '../../functions/plantillas_constancias/tallerlogoterapiaysuaplicacionenlaconsejeria.jpg';
		}
		else if($values['id_taller'] == 3){
			$plantilla = '../../functions/plantillas_constancias/tallerabcenneurocienciasdelasadicciones.jpg';
		}
		else if($values['id_taller'] == 4){
			$plantilla = '../../functions/plantillas_constancias/tallerlaperspectivadegeneroeneltratamiento.jpg';
		}
		else if($values['id_taller'] == 5){
			$plantilla = '../../functions/plantillas_constancias/tallermodelosymodalidadesmodelosymodalidades.jpg';
		}
		else if($values['id_taller'] == 6){
			$plantilla = '../../functions/plantillas_constancias/tallercaracteristicasdelahipersensibilidad.jpg';
		}
		...
		if($persona){
			...
			$nombre_reconocimiento = $values['nombre'].' '.$values['id_taller'].' '.$values['id_asistente'];
			// echo "R>>".$nombre."<br>";
		}else{
		...
		$sql = "INSERT INTO asistentes_eventos (nombre_reconocimiento, id_asistente, id_evento, id_taller) VALUES ('".$nombre_reconocimiento."','".$values['id_asistente']."',2,'".$values['id_taller']."')";
