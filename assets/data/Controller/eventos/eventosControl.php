...
if (isset($_POST["action"])) {
	...
	switch ($_POST["action"]) {
		case 'listado_eventos':
			...
			if((isset($_SESSION['usuario']) || isset($_POST['android_id_afiliado'])) && !isset($_POST['tipo'])){
				...
				for ($i=0; $i < sizeof($l_ev); $i++) { 
					...
					if(!empty($lugares_reservados) && isset( $_SESSION['usuario'] )){
					...
					$caduce = substr($l_ev[$i]["fechaLimite"],0,10);
					if(strtotime($fecha_hoy) > strtotime($caduce)){
						unset($l_ev[$i]);
					}
				}
				...
				if(isset($_POST['android_id_afiliado']) && intval($_POST['android_id_afiliado']) > 0){
					foreach($l_ev as $key => $value){
						...
						$registrado = $prospEM->consultar_alumno_evento($id, $value['idEvento']);
						if($registrado !== false){
							$l_ev[$key]['registrado'] = true;
						}else{
							$l_ev[$key]['registrado'] = false;
						}
					}
				}
		...
		case 'fechaseventos':
			if(isset($_SESSION['usuario'])){			
			echo json_encode($prospEM-> fechaseventos());
			}else{
				$array=array("error"=>'no_session');
				echo(json_encode($array));
			}
			...
			if(isset($_POST['tipo'])){
				...
				if($resp['estatus']=='ok'){
					for ($i=0; $i < sizeof($resp['data']); $i++) { 
						...
						if(isset($_SESSION['alumno'])){
							$registrado = $prospEM->consultar_alumno_evento($_SESSION['alumno']['id_prospecto'], $resp['data'][$i]['idEvento']);
							if($registrado !== false){
								$resp['data'][$i]['registrado'] = true;
							}else{
								$resp['data'][$i]['registrado'] = false;
							}
						}
					}
				}
			}
			if($_POST['tipo'] == 'proximos'){
				foreach ($resp['data'] as $ev_key => $ev_val) {
					if(strtotime(substr($ev_val['fechaLimite'], 0, 10)) < strtotime($fecha_hoy) ){
						unset($resp['data'][$ev_key]);
					}
				}
		...
		case 'eventosCalendar':
			...
			$new_arr = Array();
			foreach($resp['data'] as $datos){
				array_push($new_arr,["title"=>$datos["titulo"],"start"=>$datos["fechaE"]]); //,"end"=>$datos["fechaLimite"]
			}
		...
		case 'listasAsistencias':
			...
			break;
		case 'registrarsolicitudEventos':
			unset($_POST['action']);
			$tipo = 0;
			$resp = [];
			$fecha_hoy = date('Y-m-d');
