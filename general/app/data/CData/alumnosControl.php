...
switch ($_POST["action"]) {
	...
	case 'validarLogin':
		...
		if($alumno["data"]){
			session_start();
			if(sizeof($alumno['data']['instituciones']) > 0){
				foreach ($alumno['data']['instituciones'] as $key => $value) {
					$_SESSION['alumno_'.strtolower($value['institucion_n'])] = $alumno["data"];
				}
			}
		}
	...
	case 'cambiarpasw':
		session_start();
		if (isset($_SESSION['alumno_general'])) {//sesion afiliado siscon
		...
		if (!isset($_SESSION['alumno_general'])&&!isset($_SESSION['usuario'])) {
			echo 'sesion_destroy';
		}
		break;
	case 'cargar_pagos':
