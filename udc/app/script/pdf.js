			...
			try{
				...
				if(cursos_disp.estatus == 'ok'){
					...
					for (i = 0; i < cursos_disp.data.length; i++) {
						var foto_g = '';
						if(cursos_disp.data[i].idInstitucion == 20 && ![2, 3, 4, 5, 7].includes(parseInt(cursos_disp.data[i].estatus))){
                            //mike
    ...
    function cardsVisiblesUDC(idAlumno, id, nombreCarrera, nombreGeneracion, foto,idUser){
        $.ajax({
            ...
            data: {action: 'validar_adeudos',
                id: id,
                idAlumno: idAlumno,
                Nacion: $("#IdentificadorNacionalidad").val()},
            success: function(data){
                ...
                html_c=`<div class="col-sm-6 col-md-6 col-xl-4">
                            <div class="card mg-b-40">
                                ...
                                <button class = "btn btn-info form-control form-group mb-0 border" onClick="insertarSolicitudCred(${idUser},${id})">Elegir curso</button>
                            </div>
                        </div>`;
