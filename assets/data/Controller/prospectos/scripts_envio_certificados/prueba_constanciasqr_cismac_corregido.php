<?php
	...
	$constancias = $con->query('SELECT * 
								...
								join a_prospectos as ap on ap.idAsistente=ae.id_asistente 
								WHERE ae.id_evento = 2 and ae.nombre_reconocimiento<>"" AND nombre_reconocimiento like "%CONGRESO_CISMAC%" 
								GROUP by ae.id_asistente ORDER BY ae.id_asistente ASC')->fetchAll(PDO::FETCH_ASSOC);
	...
	foreach ($constancias as $row => $values) {
		...
		$plantilla = '';
			...
			$plantilla = '../../../functions/plantillas_constancias/cismac_corregido.jpg';
		...
		if($persona){
			...
			$nombre_reconocimiento = 'CONGRESO_CISMAC_2021_'.$nombre.'_'.$values['id_evento'].'_'.$values['id_asistente'];
			// echo "R>>".$nombre."<br>";
		}else{
		...
		$sql = "UPDATE asistentes_eventos set nombre_reconocimiento='$nombre_reconocimiento' WHERE id_asistente='".$values['id_asistente']."' AND id_evento=2 AND nombre_reconocimiento LIKE '%CONGRESO_CISMAC%';";
		            ...
		            <div style=\"padding: 5%; background-color: #F8F8F8;\">
		                <div style=\"padding: 3%; background-color: #FFFFFF; border: 1px solid #625B55; -webkit-border-radius: 10px; -moz-border-radius: 10px; border-radius: 10px;\">
		                    <div style=\"padding: 3%;\">
		                        ...
		                        <p style=\"font-family: Verdana; font-size: 16px; color: #625B55; text-align: justify;\">Queremos agradecerle por formar parte de nuestro Curso <b>".$values['titulo']."</b>, por lo tanto su <b>Constancia de Asistencia</b> ha sido generada, es por esa raz&oacute;n que adjuntamos el archivo para su descarga.</p>
